...
export class Decoder {
  ...
  readUVarInt() {
    ...
    let i = 0;
    while (
      ((currentByte = this.buffer[this.offset++]) & MOST_SIGNIFICANT_BIT) !==
      0
    ) {
      result |= (currentByte & OTHER_BITS) << i;
      i += 7;
      if (i > 28) {
        throw new KafkaJSInvalidVarIntError(
          'Invalid VarInt, must contain 5 bytes or less'
        );
      }
    }
  ...
  readVarLong() {
    ...
    do {
      if (i > 63) {
        throw new KafkaJSInvalidLongError(
          'Invalid Long, must contain 9 bytes or less'
        );
      }
      currentByte = this.buffer[this.offset++];
